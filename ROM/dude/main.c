...
void load_prg_file( char *path_to_file, uint32_t  bytes_in_file )
{
  ...
  send( base_address & 0xff );
  send( base_address >> 8 );
  // Send the number of bytes in the ROM
  send( bytes_to_load & 0xff );
  send( bytes_to_load >> 8 );
...
void  download( char  device, char *path_to_file )
{
  ...
  send( device );
  uint16_t  base_address = read16();
  uint16_t  bytes = read16();
...
int main( int argc, char *argv[])
{
  ...
  if ( mode == UPLOAD_AND_WATCH )
    while ( true)
    {
      loop();
    }
  else
    download( device, output_file );
}
